...
import com.sample.mvvm_data_binding_koin.SampleApp.Companion.appContext
...
abstract class BaseViewModel(loginRepository: LoginRepository) : ViewModel() {
    ...
    suspend fun <T : BaseResult> doNetwork(context: Context = appContext, apiFun: suspend () -> Response<T>): T? {
    ...
    private suspend fun <T : BaseResult> doApiFun(context: Context, apiFun: suspend () -> Response<T>): T? {
    ...
    private fun <T : BaseResult> doNoConnect(context: Context): T? {
        _networkUnavailableMsg.postValue("please re-connect")
        return null
    }
    ...
    private fun doOnTimeOutException(context: Context) {
        _networkExceptionTimeout.postValue("time out")
    }
